...
choreography needed for the validation path. The write section is still
wrapped defensively in case of an unexpected DB-level failure.
"""
...
def complete_sale(session: Session, cart: Cart) -> SaleReceipt:
    if not cart.items:
    ...
    resolved_lines: list[tuple[Product, Inventory, int]] = []
    ...
    try:
        # Naive local time, matching every other datetime in this codebase
        # (seed data, date.today() comparisons in services/automation) --
        # datetime.utcnow() would sort a fresh sale out of place next to
        # today's seeded orders whenever local time isn't UTC.
        now = datetime.now()
        order = Order(
        ...
        for index, (product, inventory, quantity) in enumerate(resolved_lines):
